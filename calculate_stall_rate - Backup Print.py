...
def run_analysis():
    ...
    # Kicker Share (L4)
    # Re-calculate kicker points per game for L4 window
    ...
    # Merge kicker pts to team total pts for each game
    # Need game-level team scores first
    ...
    # For defense share allowed, we need to flip it (who was the opponent?)
    ...
    def_share = share_df.groupby('opponent')['share'].mean().reset_index().rename(columns={'share': 'def_share', 'opponent': 'team'})
    ...
    final = pd.merge(final, def_pa, left_on='opponent', right_on='team', how='left')
    final = pd.merge(final, off_share, on='team', how='left')
    final = pd.merge(final, def_share, left_on='opponent', right_on='team', how='left')
    ...
    print(f"✅ Success! Generated data for Week {target_week}")
