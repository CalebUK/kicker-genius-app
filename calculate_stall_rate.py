...
def scrape_fantasy_ownership():
    ...
    # NEW URL: Stats page often has ROST column and is easier to access
    ...
    try:
        ...
        # Find Ownership Column (looking for 'rost' or 'own')
        rost_col = next((c for c in df.columns if 'rost' in c or 'own' in c), None)
        
        if not player_col or not rost_col:
        ...
        # Clean Name: "Brandon Aubrey (DAL)" -> "B.Aubrey"
        def clean_name_to_match(val):
            if not isinstance(val, str): return val
            # Remove team initials in parens
            name_part = val.split('(')[0].strip()
            parts = name_part.split(' ')
            if len(parts) >= 2:
                first = parts[0]
                last = parts[-1]
                if len(first) > 0: return f"{first[0]}.{last}"
            return name_part

        df['match_name'] = df['full_name'].apply(clean_name_to_match)
        
        # Clean Percentage: "95.2%" -> 95.2
        def clean_pct(val):
...
def run_analysis():
    try:
        ...
        # Updated: Scrape from new stats URL
